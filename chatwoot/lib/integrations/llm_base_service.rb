class Integrations::LlmBaseService
  ...
  # Fork addition: `hook.settings['api_base']` lets a single account's AI
  # Assist hook override the installation-wide `CAPTAIN_OPEN_AI_ENDPOINT`
  # (e.g. point one tenant at a different OpenAI-compatible endpoint than
  # everyone else). Optional — blank/absent falls through to the
  # installation config exactly like before this patch, so retrocompatible
  # by construction. Upstream already made the installation-wide endpoint
  # configurable between v3.16.0 and v4.16.2 (via Super Admin > Captain),
  # which covers our main multitenant case (single shared 9Router, only the
  # api_key differs per tenant) — this only adds the narrower per-hook
  # override on top, for the rarer case a specific tenant needs its own URL.
  def api_base
    endpoint = hook.settings['api_base'].to_s.strip.presence ||
